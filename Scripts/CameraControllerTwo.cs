...
public class CameraControllerTwo : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Camera follow (follows the parent Player object)
        // Vector3 playerForward = playerTransform.forward;
        // playerForward.y = 0f;
        // playerForward.Normalize();
        // Vector3 playerRight = playerTransform.right.normalized;

        // Vector3 desiredForward = playerForward;
        // Vector3 desiredRight = playerRight;

        // float t = Time.deltaTime * 5f; // Adjust the interpolation speed as needed
        // transform.forward = Vector3.Slerp(transform.forward, desiredForward, t);
        // transform.right = Vector3.Slerp(transform.right, desiredRight, t);

        // targetForward = player.transform.forward;
        // targetRight = player.transform.right;

        // transform.forward = Vector3.Slerp(transform.forward, targetForward, Time.deltaTime * rotateSpeed);
        // transform.right = Vector3.Slerp(transform.right, targetRight, Time.deltaTime * rotateSpeed);
